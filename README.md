...
### 地点数据配置（src/data/places.json）

统一维护各地点的地理坐标与图标，格式如下：
...
- 新增地点时，在 `places.json` 中添加一项即可，`icon` 指向 `public/` 下的图片（以 `/` 开头）。
- Map.vue 会自动读取并在地图上生成对应标记与标签。
- 已内置对 `library`、`gym`、`canteen` 三个 `id` 的点击后弹窗联动。
