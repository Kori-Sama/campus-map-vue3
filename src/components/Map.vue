<template>
    <div class="app-root">
        <!-- 位置详情组件 -->
        <LibraryModal :show="showLibraryModal" @close="showLibraryModal = false" @navigate="navigateToLibrary" />
        <GymModal :show="showGymModal" @close="showGymModal = false" @navigate="navigateToGym" />
        <CanteenModal :show="showCanteenModal" @close="showCanteenModal = false" @navigate="navigateToCanteen" />
        ...
        <div class="map-wrapper">
            ...
            <div class="top-controls">
                ...
                <button :class="['control', { active: currentType === '2d' }]" @click="setMapMode('2d')">2D</button>
                <button :class="['control', { active: currentType === '3d' }]" @click="setMapMode('3d')">3D</button>
            ...
            <div class="map-ctrls">
                ...
                <button class="lock-btn" @click="lockCurrentLocation" title="锁定当前位置">📍</button>
                <div class="zoom">
            ...
            <div class="info-window" v-if="info.show">
                <div class="info-window-header">
                    <h3>{{ info.title }}</h3>
                    <button class="info-window-close" @click="info.show = false">✕</button>
                </div>
                <p>{{ info.desc }}</p>
            </div>
            ...
            <div v-if="showNavModal" class="nav-modal">
                <div class="nav-modal-content">
                    ...
                    <div class="nav-modal-header">
                        <h3>地图导航</h3>
                        <button class="nav-modal-close" @click="showNavModal = false">✕</button>
                    </div>

                    <!-- 步行/驾车选择 -->
                    <div class="nav-mode-tabs">
                        <button :class="['nav-tab', { active: travelMode === 'walking' }]"
                            @click="travelMode = 'walking'">
                            🚶 步行
                        </button>
                        <button :class="['nav-tab', { active: travelMode === 'driving' }]"
                    ...
                    <div class="nav-form">
                        ...
                        <div class="nav-input-group">
                            ...
                            <div class="nav-search-box">
                                <input v-model="navOrigin" placeholder="输入起点地址（如：图书馆、一号教学楼）" class="nav-input" />
                            ...
                            <ul v-if="originSearchResults.length" class="nav-results-list">
                                <li v-for="(result, idx) in originSearchResults" :key="'origin-' + idx"
                                    ...
                                    <button class="nav-result-btn" @click="selectOrigin(result)">设为起点</button>
                                </li>
                            </ul>
                        </div>
                        ...
                        <div class="nav-input-group">
                            ...
                            <div class="nav-search-box">
                                <input v-model="navDestination" placeholder="输入终点地址（如：食堂、操场）" class="nav-input" />
                                <button class="nav-search-btn" @click="searchDestination">🔍</button>
                            </div>
                            ...
                            <ul v-if="destSearchResults.length" class="nav-results-list">
                                <li v-for="(result, idx) in destSearchResults" :key="'dest-' + idx"
                                    class="nav-result-item">
                                    <div class="nav-result-info">
                                        <div class="nav-result-title">{{ result.title }}</div>
                                        <div class="nav-result-addr">{{ result.address }}</div>
                                    </div>
                                    <button class="nav-result-btn" @click="selectDestination(result)">设为终点</button>
                                </li>
                            </ul>
                        </div>

                        <div class="nav-hint">💡 输入地址或建筑名称，点击搜索按钮查看结果，然后选择确认</div>
                    </div>

                    <!-- 蓝色开始按钮 -->
                    <button class="nav-btn-start" @click="startNavigation">🚀 开始导航</button>
                </div>
            </div>
        </div>
    </div>
...
const routeMarkers = [];
// POI 坐标（示例，经纬度可按需调整）
const libraryPoint = new BMapGL.Point(103.99198021119682, 30.772101164459176); // 图书馆
const gymPoint = new BMapGL.Point(103.989891, 30.769714); // 体育馆示例坐标
const canteenPoint = new BMapGL.Point(103.985733, 30.771923); // 食堂示例坐标
...
function initMap(type = '2d') {
    ...
    // 清理此前的 markers 列表
    try { allMarkers.length = 0; } catch (e) { }
    ...
    try {
        document.addEventListener('click', function (e) {
            try {
                ...
                // 检查点击是否在地图容器内
                if (clickX >= mapRect.left && clickX <= mapRect.right &&
                    clickY >= mapRect.top && clickY <= mapRect.bottom) {
    ...
    try {
        map.addEventListener('click', function (e) {
            try {
                console.log('map click 事件触发，已注册 marker 数：', allMarkers.length);
                if (!e || !e.point) {
        ...
        geolocation.getCurrentPosition(function (r) {
            if (this.getStatus() === BMAP_STATUS_SUCCESS) {
                ...
                const currentHandler = function () {
                    ...
                    info.desc = '你在这里';
                    info.show = true;
                    try { map.centerAndZoom(currentPoint, 18); } catch (e) { }
                };
                currentMarker._onClick = currentHandler;
                currentMarker.addEventListener('click', currentHandler);
                try { allMarkers.push(currentMarker); } catch (e) { }
                // 为当前位置添加一个可点击标签（方便移动设备/触控点击）
                try { createClickableLabel(currentPoint, '您在这里', currentHandler); } catch (e) { }
...
function addMarkers() {
    buildings.forEach(b => {
        ...
        const handler = function () {
            ...
            info.desc = b.desc;
            info.show = true;
            try { map.centerAndZoom(point, 18); } catch (e) { }
        };
        ...
        } catch (err) { console.warn('marker click 绑定失败', err); }
        try {
            marker.addEventListener('mousedown', function (e) {
                console.log('marker mousedown 事件触发', b.name, e);
                handler();
            });
        } catch (err) { console.warn('marker mousedown 绑定失败', err); }
        try {
            marker.addEventListener('mouseup', function (e) {
                console.log('marker mouseup 事件触发', b.name, e);
                handler();
            });
        } catch (err) { console.warn('marker mouseup 绑定失败', err); }
        try { allMarkers.push(marker); } catch (e) { }
        // 添加纯视觉标签（不绑定事件，因为 BMapGL Label 事件不可靠）
        try {
            const label = new BMapGL.Label(b.name || '', { position: point, offset: new BMapGL.Size(10, -28) });
            label.setStyle({ cursor: 'pointer', background: 'rgba(255,255,255,0.92)', padding: '4px 8px', borderRadius: '8px', fontSize: '12px', color: '#222', boxShadow: '0 6px 18px rgba(0,0,0,0.12)' });
            map.addOverlay(label);
            // 为 label 创建虚拟 marker 用于 map click 回退
            const virtualMarker = {
                getPosition: function () { return point; },
                _onClick: handler
            };
            try { allMarkers.push(virtualMarker); } catch (e) { }
        } catch (e) {
            // label 创建失败则忽略
        }
    });
...
function addLibraryMarker() {
    try {
        const marker = new BMapGL.Marker(libraryPoint, {
            icon: new BMapGL.Icon('/图书馆.jpg', new BMapGL.Size(50, 50), {
                imageOffset: new BMapGL.Size(0, 0)
            })
        });

        const handler = function () {
            info.title = '📚 西南交通大学图书馆';
            info.desc = '点击查看详情';
            info.show = true;
            showLibraryModal.value = true;
            try { map.centerAndZoom(libraryPoint, 18); } catch (e) { }
        };
        ...
        // 添加标签
        try {
            createClickableLabel(libraryPoint, '📚 图书馆', handler);
        } catch (e) { }
    } catch (e) {
        console.warn('添加图书馆标记失败:', e);
    }
}

// 添加体育馆标记
function addGymMarker() {
    try {
        const marker = new BMapGL.Marker(gymPoint, {
            icon: new BMapGL.Icon('/gym.png', new BMapGL.Size(44, 44), { imageOffset: new BMapGL.Size(0, 0) })
        });
        const handler = function () {
            info.title = '🏟️ 体育馆';
            ...
            try { map.centerAndZoom(gymPoint, 18); } catch (e) { }
        };
        marker._onClick = handler;
        marker.addEventListener('click', handler);
        map.addOverlay(marker);
        try { allMarkers.push(marker); } catch (e) { }
        try { createClickableLabel(gymPoint, '🏟️ 体育馆', handler); } catch (e) { }
    } catch (e) {
        console.warn('添加体育馆标记失败:', e);
    }
}

// 添加食堂标记
function addCanteenMarker() {
    try {
        const marker = new BMapGL.Marker(canteenPoint, {
            icon: new BMapGL.Icon('/canteen.png', new BMapGL.Size(44, 44), { imageOffset: new BMapGL.Size(0, 0) })
        });
        const handler = function () {
            info.title = '🍽️ 校内食堂';
            ...
            try { map.centerAndZoom(canteenPoint, 18); } catch (e) { }
        };
        marker._onClick = handler;
        marker.addEventListener('click', handler);
        map.addOverlay(marker);
        try { allMarkers.push(marker); } catch (e) { }
        try { createClickableLabel(canteenPoint, '🍽️ 食堂', handler); } catch (e) { }
    } catch (e) {
        console.warn('添加食堂标记失败:', e);
    }
}
...
function createClickableLabel(point, text, handler) {
    try {
        const label = new BMapGL.Label(text || '', { position: point, offset: new BMapGL.Size(10, -28) });
        label.setStyle({ cursor: 'pointer', background: 'rgba(255,255,255,0.92)', padding: '4px 8px', borderRadius: '8px', fontSize: '12px', color: '#222', boxShadow: '0 6px 18px rgba(0,0,0,0.12)' });
...
function addBuilding() {
    ...
    marker.addEventListener('click', handler);
    try { allMarkers.push(marker); } catch (e) { }
    try { createClickableLabel(point, building.name, handler); } catch (e) { }
...
async function startNavigation() {
    ...
    const originStr = (selectedOrigin.point ? selectedOrigin.name : navOrigin.value) || '';
    const destStr = (selectedDestination.point ? selectedDestination.name : navDestination.value) || '';
...
function clearNavigation() {
    try {
        if (currentRoute && typeof currentRoute.clearResults === 'function') currentRoute.clearResults();
    } catch (e) { /* ignore */ }
    // remove any markers we may have added
    routeMarkers.forEach(m => { try { map.removeOverlay(m); } catch (e) { } });
...
function loadCampusPOIs() {
    ...
    // 常见分类（可根据需要扩展）
    const categories = [
        '图书馆', '教学楼', '食堂', '宿舍', '体育场', '实验室', '行政楼', '银行', '超市', '医院', '公交站'
    ];
        ...
        campusPoiSearch = new BMapGL.LocalSearch(map, {
            ...
            onSearchComplete: function (results) {
                ...
                for (let i = 0; i < num; i++) {
                    ...
                    const key = `${poi.title}_${poi.point.lng.toFixed(6)}_${poi.point.lat.toFixed(6)}`;
                    if (poiSet.has(key)) continue;
                    poiSet.add(key);

                    const marker = new BMapGL.Marker(poi.point);
                    const infoText = `${poi.title}${poi.address ? ' — ' + poi.address : ''}`;
                    map.addOverlay(marker);
                    const handler = function () {
                        info.title = poi.title || '';
                        info.desc = poi.address || '';
                        info.show = true;
                        try { map.centerAndZoom(poi.point, 18); } catch (e) { }
                    };
                    marker._onClick = handler;
                    marker.addEventListener('click', handler);
                    ...
                    try { allMarkers.push(marker); } catch (e) { }
                    // 同时添加文本标签以增加点击命中面
                    try { const lbl = createClickableLabel(poi.point, poi.title || '', handler); if (lbl) campusPoiOverlays.push(lbl); } catch (e) { }
                }
            }
        });
    }
...
function loadAllTeachingBuildings() {
    ...
    if (window[importKey]) {
        console.warn('已执行过教学楼导入，若需重新导入请刷新页面或先清理 buildings');
        return;
    }
        ...
        campusPoiSearch = new BMapGL.LocalSearch(map, {
            ...
            onSearchComplete: function (results) {
                ...
                for (let i = 0; i < num; i++) {
                    ...
                    // 将 POI 转为教学楼条目并添加
                    const b = { name: poi.title || '教学楼', lng: poi.point.lng, lat: poi.point.lat, desc: poi.address || '' };
    ...
    // 顺序执行查询
    queries.forEach(q => {
        try { campusPoiSearch.search(q); } catch (e) { console.warn('search failed', q, e); }
    });
...
function debugListMarkers() {
    try {
        ...
        for (let i = 0; i < allMarkers.length; i++) {
            try {
                const m = allMarkers[i];
                const pos = (typeof m.getPosition === 'function') ? m.getPosition() : (m.position || null);
                console.log(i, m, pos && { lng: pos.lng, lat: pos.lat });
            } catch (err) { }
        }
    } catch (e) { console.warn('debugListMarkers failed', e); }
}
...
function navigateToLibrary() {
    ...
    // 自动设置终点为图书馆
    selectedDestination.name = '西南交通大学图书馆';
    selectedDestination.point = libraryPoint;
    navDestination.value = '西南交通大学图书馆';
}
...
function navigateToGym() {
    ...
    showNavModal.value = true;
    selectedDestination.name = '体育馆';
    selectedDestination.point = gymPoint;
    navDestination.value = '体育馆';
}
...
function navigateToCanteen() {
    ...
    showNavModal.value = true;
    selectedDestination.name = '校内食堂';
    selectedDestination.point = canteenPoint;
    navDestination.value = '校内食堂';
}
    ...
    const onSidebarSearch = function (e) {
        try {
            ...
            if (typeof BMapGL === 'undefined') {
                ...
                document.dispatchEvent(new CustomEvent('sidebar-search-results', { detail: { results: [] } }));
                return;
            }
                    ...
                    onSearchComplete: function (results) {
                        try {
                            ...
                            if (!results) {
                                ...
                                document.dispatchEvent(new CustomEvent('sidebar-search-results', { detail: { results: out } }));
                                return;
                            }
                            const count = results.getCurrentNumPois ? results.getCurrentNumPois() : (results.getNumPois ? results.getNumPois() : 0);
                            console.log('[Map] LocalSearch 完成，POI 数量:', count);
                            for (let i = 0; i < count; i++) {
                                try {
                                    const poi = results.getPoi(i);
                                    if (!poi) continue;
                                    out.push({ title: poi.title || poi.name || '', address: poi.address || '', point: poi.point ? { lng: poi.point.lng, lat: poi.point.lat } : null });
                                } catch (ie) { }
                            }
                            // 如果 getPoi 不可用，尝试 results.getPoi() 数组形式
                            if (out.length === 0 && Array.isArray(results.getPoi ? results.getPoi() : [])) {
                                const arr = results.getPoi();
                                arr.forEach(poi => out.push({ title: poi.title || poi.name || '', address: poi.address || '', point: poi.point ? { lng: poi.point.lng, lat: poi.point.lat } : null }));
                            }
            ...
            try { console.log('[Map] 调用 localSearch.search ->', kw); localSearch.search(kw); } catch (err) { console.warn('localSearch.search 调用失败', err); document.dispatchEvent(new CustomEvent('sidebar-search-results', { detail: { results: [] } })); }
        } catch (err) {
